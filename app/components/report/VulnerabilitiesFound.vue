...
const sevIconTint = (s: Severity) =>
  s === "CRITICAL" ? "text-rose-500" : s === "HIGH" ? "text-amber-500" : "text-yellow-500";
</script>
      ...
      >
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="flex items-center gap-2">
            <component :is="sevIcon(v.severity)" class="w-4 h-4" :class="sevIconTint(v.severity)" />
            <div class="font-medium text-sm">{{ v.title }}</div>
          </div>
          <span class="text-[11px] rounded-full px-2 py-0.5" :class="sevPill(v.severity)">
            {{ v.severity }}
          </span>
        </div>
